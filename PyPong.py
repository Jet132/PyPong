...
class PyPong():
    ...
    def _do_action(self, action, player_id):
        ...
        if(action == ACTION_UP):
            ...
            raise ValueError(
                "Illegal action: {} from player {}".format(action, player_id))
        ...
        if(player_id == 0):
            ...
            raise ValueError(
                "player_id must be either 0 or 1 but was {}".format(player_id))
    ...
    def _player_catch(self, player_id):
        ...
        if(self.ball[1]+self.ball_size >= player and self.ball[1] <= player+self.player_size):
            ...
            self.ball_v[1] = self.ball_v[1] - \
                self._y_distance_to_ball(player) * 0.1
    ...
    def get_states(self, player=-1):
        if (player == 0):
            return self._pack_data()[0]
        elif (player == 0):
            return self._pack_data()[1]
        return self._pack_data()

    def execute(self, actions, player=-1):
        """
        ...
        if not self._do_action(actions[0], 0):
            # optional illegal action penalty
            pass
        if not self._do_action(actions[1], 1):
            # optional illegal action penalty
            pass
